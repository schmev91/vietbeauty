<div class="d-flex gap-3">
    <div class="fields flex-grow-1 ">
        <!-- fields start -->
        <!-- TÊN DANH MỤC -->
        <?php initInput('text', 'ten_dm', 'Tên danh mục') ?>


        <!-- INPUT UPLOAD PRODUCTIMG -->
        <div class="mb-3">
            <label for="formFile" class="form-label">Ảnh sản phẩm</label>
            <input id="upload-categoryImg" name="hinh_dm" class="form-control" type="file" id="formFile">
        </div>

        <!-- fields end -->
    </div>
    <label for="upload-categoryImg" class=" flex-grow-0 " style="height: fit-content;">
...
<script>
    document.getElementById('upload-categoryImg').addEventListener('change', function(event) {
